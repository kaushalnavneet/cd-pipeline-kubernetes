...
if ! helm list ${IDS_STAGE_NAME}; then
  ...
  helm install --name ${IDS_STAGE_NAME} ${CHART_DIR} --namespace ${CHART_NAMESPACE} --set ${ENVIRONMENT}.enabled=true --set pipeline.image.tag=${APPLICATION_VERSION} --set pipeline.image.repository=${IMAGE_NAME}
  else
