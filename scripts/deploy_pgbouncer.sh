...
kubectl delete secret ${TARGET}-postgres-secret
echo "Creating '${TARGET}-postgres-secret' secret..."
kubectl create secret generic ${TARGET}-postgres-secret --from-literal=postgres-password=${PG_PASSWORD}

kubectl delete secret ${TARGET}-pgbouncer-secret
...
kubectl create secret generic ${TARGET}-pgbouncer-secret  --from-file=userlist.txt --from-file=.pgpass
rm -f userlist.txt
rm -f .pgpass
